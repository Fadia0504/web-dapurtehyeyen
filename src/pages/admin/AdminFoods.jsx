...
import {
  ...
  XMarkIcon, CloudArrowUpIcon, ChevronDownIcon, ArrowRightOnRectangleIcon
...
export default function AdminFoods() {
  ...
  // Konversi harga: hapus titik/koma ribuan, jadikan number
  ...
  const handleSave = async () => {
    ...
    if (isNaN(finalPrice) || finalPrice <= 0) {
      alert('Harga tidak valid! Masukkan angka tanpa titik atau koma. Contoh: 15000')
      return
    }
  ...
  const filtered = foods.filter((f) => {
    ...
    const matchStatus =
      filterStatus === '' ? true : filterStatus === 'aktif' ? f.is_available : !f.is_available
  ...
  const pricePreview = form.price
    ? Number(String(form.price).replace(/[^0-9]/g, '')).toLocaleString('id-ID')
    : ''
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                            food.categories?.name === 'Paket Catering'
                              ? 'bg-purple-100 text-purple-600'
                              : food.categories?.name === 'Ala Carte'
                              ? 'bg-blue-100 text-blue-600'
                              : food.categories?.name === 'Minuman'
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-6 py-4 border-t border-gray-50">
                <p className="text-sm text-gray-400">
                  ...
                  {Math.min(page * perPage, filtered.length)} dari {filtered.length} menu
                </p>
            ...
            <div className="p-6 space-y-5">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-gray-700 block mb-1">
                      Harga Jual * <span className="text-gray-400 font-normal">(ketik angka saja, tanpa titik)</span>
                    </label>
                    <div className="relative">
              ...
              <div>
                ...
                <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
                  ...
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all shadow-sm ${form.is_available ? 'left-6' : 'left-0.5'}`}
                    />
                  </button>
                </div>
              </div>
